...
const searchInputClearX = document.querySelector("#search-input-clear");
...
function AddToDo(e) {
  ...
  const current12Hours = time.toLocaleString("en-US", {
    hour: "numeric",
    minute: "numeric",
    second: "numeric",
    hour12: true,
...
//Search input
searchInputClearX.addEventListener("click", () => {
  searchInput.value = "";
});
...
searchInput.addEventListener("keyup", () => {
  ...
  const allTodos = document.querySelectorAll(".collection-item");

  allTodos.forEach((todo) => {
    console.log(todo.firstChild);
    if (
      todo.firstChild.textContent.toLowerCase().indexOf(searchedWord) !== -1
    ) {
